...
modelIPDNMR_Relapse<-function(){
  ...
  for (i in 1:Np){
    outcome[i]~dbern(p[i])
    ###formula
    logit(p[i])<-u[studyid[i]] + d[studyid[i], arm[i]] + g[studyid[i]]*(Risk[i]) + g.w[studyid[i],arm[i]]*(Risk[i])
  }
  ...
  for (i in 1:Nnew){
    for(j in 1:nt){
      logitp[i,j]<-logitpplacebo+ delta[j]+ gamma*(logitRisknew[i,1])+ gamma.w[j]*(logitRisknew[i,1])
    }
  }
